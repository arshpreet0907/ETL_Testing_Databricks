...
def apply_transforms(
    ...
    if dialect == 'snowflake':
        df = df.withColumn('assembly_start_ts', F.when(F.col('assembly_start_ts').cast(StringType()).startswith('0000-00-00'), F.lit(None)).otherwise(F.from_utc_timestamp(F.col('assembly_start_ts').cast(TimestampType()), 'Asia/Kolkata')))
        logger.debug('  [sf-coerce] assembly_start_ts: datetime/timestamp -> TimestampType (UTC->IST), null zero-datetimes')
        df = df.withColumn('assembly_end_ts', F.when(F.col('assembly_end_ts').cast(StringType()).startswith('0000-00-00'), F.lit(None)).otherwise(F.from_utc_timestamp(F.col('assembly_end_ts').cast(TimestampType()), 'Asia/Kolkata')))
        logger.debug('  [sf-coerce] assembly_end_ts: datetime/timestamp -> TimestampType (UTC->IST), null zero-datetimes')
