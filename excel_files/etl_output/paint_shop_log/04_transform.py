...
def apply_transforms(
    ...
    if dialect == 'snowflake':
        ...
        df = df.withColumn('process_start_ts', F.from_utc_timestamp(F.col('process_start_ts'), 'Asia/Kolkata'))
        logger.debug('  [sf-coerce] process_start_ts: datetime/timestamp -> TimestampType (IST adjusted)')
        ...
        # Source timestamps are IST (from MySQL); Snowflake stores them converted to IST representation
        df = df.withColumn('process_end_ts', F.from_utc_timestamp(F.col('process_end_ts'), 'Asia/Kolkata'))
        logger.debug('  [sf-coerce] process_end_ts: datetime/timestamp -> TimestampType (IST adjusted)')
